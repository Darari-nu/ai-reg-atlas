...
function buildPrompt(batch, feedList, sourceDomains) {
...
- relevant: AI（人工知能）を対象とする法規制・ガイドライン・施行令・公的ガイダンス、またはAIの開発・利用に具体的に触れる規制当局の決定・執行か。AIに触れない一般の個人情報保護・通信・放送・生命科学・サイバー規制は false（AI固有の規定を含む場合だけ true）。ディープフェイク・AI生成物・自動化された意思決定に関する規定は、刑法・選挙法・消費者法などの中にあっても AI固有の規定として true。ニュース解説のみ・製品発表・株価も false。source_kind が media（報道）の候補は、さらに厳しく判定する: 対象国自身の政府・議会・規制当局・裁判所が主体となった法規制の動き（法案の提出・審議・成立、施行、規則・ガイドラインの公表、意見募集、執行・処分、判決）を報じる記事だけ true。論説・寄稿・コラム・解説・インタビュー・業界団体や専門家の提言、政治家・首脳・外交官の発言や姿勢表明だけのもの、国際会議での主張だけのものは false
- country: その規制・決定を行う国・地域の対象国コード（複数可・対象外なら除外）。記事で言及されるだけの国や、country_hint の国をそのまま付けない（country_hint は検索に使った国の目安にすぎない）
...
候補: ${JSON.stringify(buildTriagePayload(batch, sourceDomains))}`;
}

function callTriage(batch, feedList, sourceDomains) {
  return geminiJSONWithRetry({
    model: MODEL_TRIAGE,
    prompt: buildPrompt(batch, feedList, sourceDomains),
...
async function runSecondLook(secondLook, feedList, seenUrls, sourceDomains) {
  const rescued = [];
  const stillIrrelevant = [];
  const batches = chunk(secondLook, BATCH_SIZE);
  for (const [bi, batch] of batches.entries()) {
    try {
      const verdicts = await callTriage(batch, feedList, sourceDomains);
...
async function main() {
  ...
  if (secondLook.length > 0 && !stopped) {
    triaged.push(...(await runSecondLook(secondLook, feedList, seenUrls, sourceDomains)));
