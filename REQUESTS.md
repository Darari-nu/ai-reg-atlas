# REQUESTS.md — AI Reg Atlas 完全実装仕様書 v2.1
...
- **v2.1（2026-06-12・施主承認済み）**: スマホUI改修。①スマホでも地球儀を表示する「昇る地球」ヒーロー（地球儀非表示フォールバックを廃止、WebGL非対応時のみ非表示） ②スマホ専用の下部固定タブバー（トップ/比較表/年表/更新。ヘッダーナビはスマホではAboutのみ） ③6カ国ステータスはスマホで2列グリッドカード（国旗大・状態色ドット） ④NEW1件目に金の左罫線。§6-3/§6-5/§12-2に反映済み
- v2.0: 初版（一撃実装仕様）
...
- スマホ（<768px・v2.1）：**「昇る地球」ヒーロー**＝画面下から半分覗く地球儀の上にタグラインを重ねる（下端は紺へグラデーションフェード）→ NEW3件 → 6カ国カード（**2列グリッド**：国旗大きく・状態は色ドット＋パルス・規制名1行truncate）
- スマホには**下部固定タブバー**（◍トップ / ▦比較表 / ≡年表 / ⚡更新。アクティブは金＋下線、safe-area-inset対応）。ヘッダーナビはスマホではAboutのみ
- WebGL非対応環境のみ地球儀なしフォールバック（タグライン→NEW→6カ国カード）
- PC用/スマホ用の地球儀2インスタンスはmatchMediaで表示側のみWebGL初期化
...
1. **StatusBar**：6カ国チップ（国旗＋規制名＋状態バッジ）。スマホでは2列グリッドカード（v2.1）
1. **Footer**：免責／「要約生成: Gemini API」明示／`LAST SWEEP {meta.json}`（全ページ）
1. **TabBar**（v2.1）：スマホ専用の下部固定ナビ（トップ/比較表/年表/更新）。md以上では非表示。本文側に `pb-16 md:pb-0` を確保
...
- [ ] スマホ（<768px）で「昇る地球」ヒーロー＋下部タブバー＋2列国カードに切替（v2.1）。タブバーが本文・フッターを隠さない
